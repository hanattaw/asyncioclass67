...
class AsyncDatbaseConnection:
    def __init__(self, db_name):
        self.db_name =db_name

    async def __aenter__(self):
        print(f"Connecting to the database {self.db_name}...")
        await asyncio.sleep(1) #Simmulate async connection setup
...
async def main():
    async with AsyncDatbaseConnection("test_db") as db:
        data = await db.fetch_data()
        print(f"Fetched data: {data}")

# Running the async maine function
asyncio.run(main())
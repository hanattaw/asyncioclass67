...
async def fetch_hacker_news():
    async with aiohttp.ClientSession() as session:
        ...
        url2 = f"https://hacker-news.firebaseio.com/v0/item/2659683.json?print=pretty"
        task2 = asyncio.create_task(get_api(session, url))

        news1 = await task1 # wait till task 1 is complete
        news2 = await task2 # wait till task 1 is complete
